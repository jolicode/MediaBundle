...
namespace JoliCode\MediaBundle\Bridge\Sylius;
...
final class JoliMediaSyliusBundle extends AbstractBundle
{
                ...
                ->addDefaultsIfNotSet()
                    ->children()
                        ...
                        ->booleanNode('show_variations_stored')
                            ->defaultFalse()
                            ->info('If true, shows whether a variation is already stored in the media library or not.')
                        ->end()
                    ->end()
                ->end()
            ->end()
    ...
    public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        if (!isset($bundles['JoliMediaBundle'])) {
            throw new \RuntimeException('The "JoliMediaBundle" bundle needs to be registered in order to use JoliMediaSyliusBundle.');
        }

        if (!isset($bundles['SyliusBootstrapAdminUiBundle']) && !isset($bundles['SyliusAdminBundle'])) {
            throw new \RuntimeException('The "SyliusBootstrapAdminUiBundle" or the "SyliusAdminBundle" bundle needs to be registered in order to use JoliMediaSyliusBundle.');
        }
    ...
    public function prependExtension(ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        if (isset($joliMediaConfig[0]['default_library'])) {
            ...
            throw new \RuntimeException('It is required to define a library in the JoliMediaBundle configuration before using the JoliMediaSyliusBundle.');
        }

        $builder->prependExtensionConfig('twig', [
            'form_themes' => [
                '@JoliMediaSylius/admin/form/form_theme.html.twig',
            ],
        ]);
        ...
        $builder->prependExtensionConfig('framework', [
            'assets' => [
                'packages' => [
                    'joli_media_sylius_admin' => [
                        'base_path' => '/bundles/JoliMediaSylius',
                    ],
                ],
            ],
        ]);
