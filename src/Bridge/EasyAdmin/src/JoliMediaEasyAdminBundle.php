...
class JoliMediaEasyAdminBundle extends AbstractBundle
{
        ...
        $definition->rootNode()
            ->children()
                ...
                ->arrayNode('text_editor')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('variation')
                            ->defaultNull()
                            ->info('Name of the variation to insert when a media is picked from a TextEditorField. Leave empty to insert the original media.')
                            ->validate()
                                ->ifTrue(static fn (mixed $value): bool => null !== $value && (!\is_string($value) || '' === $value))
                                ->thenInvalid('The "joli_media_easy_admin.text_editor.variation" option must be a non-empty string or null.')
                            ->end()
                        ->end()
                    ->end()
                ->end()
    ...
    public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        $container->services()
            ...
            ->arg('$textEditorVariation', $config['text_editor']['variation'])
        ;
