...
abstract readonly class AbstractProcessor extends AbstractProcessCreator implements ProcessorInterface
{
    ...
    protected function checkOutputFormat(string $outputFormat): void
    {
        if (!$this->canProcessOutputFormat($outputFormat)) {
            throw new \InvalidArgumentException(\sprintf('The processor "%s" cannot process "%s" files. Available formats are: %s', static::class, $outputFormat, implode(', ', array_map(static fn (Format $value) => $value->value, $this->getProcessableOutputFormats()))));
        }
    }
